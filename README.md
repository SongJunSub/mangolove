...
>
> 커버리지는 스킬 호출의 `args` 로 범위를 좁힙니다. `/code-review 1952`(원격 PR), PR URL, 레포 밖 경로(다른 worktree)를 리뷰한 스킬은 이 워킹트리를 **전혀 커버하지 않습니다**. 실재하는 경로를 짚었으면 그 경로만 커버합니다. 강도만 주거나(`high`) 자유 서술이면 스킬 기본 범위대로 전체를 커버합니다. 산문은 해석하지 않습니다: 해석하면 그게 다시 모델 판단이 됩니다.
>
