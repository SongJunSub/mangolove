...
>
> 커버리지는 스킬 호출의 `args` 로 범위를 좁힙니다. **인자 전체를 알아볼 수 있어야** 범위를 인정합니다. 인자가 없거나 강도(`low`/`medium`/`high`/`max`/`xhigh`/`ultra`)뿐이면 스킬 기본 범위대로 전체를 커버하고, 이 트리의 실재하는 경로를 짚었으면 그 경로만 커버합니다. **그 밖의 토큰이 하나라도 있으면 아무 것도 커버하지 않습니다**: PR 번호와 링크, 다른 worktree 경로, 티켓 번호, 브랜치 이름, 자유 서술이 여기 해당합니다.
>
