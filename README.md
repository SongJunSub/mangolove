...
>
> 커버리지는 스킬 호출의 `args` 로 범위를 좁힙니다. **명확히 다른 대상을 가리키는 토큰**이 있으면 그 호출은 이 트리를 전혀 커버하지 않습니다: PR 번호와 링크, `#번호`, `PR`/`MR` 낱말과 숫자의 조합, git ref 로 풀리는 브랜치와 태그, 레포 밖 절대경로, 값을 품은 `--옵션`. 전부 형태로 확정되는 신호입니다. 이 트리의 실재하는 경로를 짚었으면 그 경로만 커버하고, 그 밖의 낱말(강도 지정, 산문)은 무시합니다. 가리키는 것이 아무것도 없으면 스킬 기본 범위대로 전체를 커버합니다.
>
