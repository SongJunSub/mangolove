...
>
> 커버리지는 스킬 호출의 `args` 로 범위를 좁힙니다. **인자 전체를 알아볼 수 있어야** 범위를 인정합니다. 인자가 없거나 강도(`low`/`medium`/`high`/`max`/`xhigh`/`ultra`)뿐이면 스킬 기본 범위대로 전체를 커버하고, 이 트리의 실재하는 경로를 짚었으면 그 경로만 커버합니다. **그 밖의 토큰이 하나라도 있으면 아무 것도 커버하지 않습니다**: PR 번호와 링크, 다른 worktree 경로, 티켓 번호, 브랜치와 태그 이름, 자유 서술, 그리고 알려진 비-범위 플래그가 아닌 옵션(`--pr=1952` 등)이 여기 해당합니다.
>
> 원격 참조(PR 번호, URL, git ref 로 풀리는 브랜치와 태그) 판정은 경로 실재 검사보다 **먼저** 합니다. 순서를 뒤집으면 이름이 겹치는 로컬 경로가 원격 참조를 가립니다: 브랜치에 `1952/` 나 `main/` 디렉토리를 심어 두면 `/code-review 1952` 가 그 디렉토리를 봤다고 기록됩니다.
>
> 기본값이 "전체 커버"가 아니라 "커버 안 함"인 것은 의도입니다. 모르는 토큰을 수식어로 넘겼더니 `review everything except lib` 의 `lib` 을 커버 대상으로 읽고(부정을 긍정으로), `/code-review main` 처럼 다른 브랜치를 본 리뷰가 워킹트리 전체를 통과시켰습니다. 과대 인정은 조용하고 감사 기록도 남지 않지만, 과소 인정은 눈에 보이고 한 번 다시 돌리면 됩니다. 차단 메시지가 그 방법을 안내합니다.
>
> 게이트 상태(원장, 커버리지)는 워킹트리가 아니라 `.git/mangolove/` 아래에 둡니다. `.gitignore` 는 `git add -f` 를 막지 못하므로, 워킹트리에 두면 적대적 브랜치가 위조본을 커밋해 두고 그 브랜치를 checkout 한 사람에게 배달할 수 있습니다. 터미널 경로는 세션 ID 가 없어 디스크의 원장을 그대로 믿으므로 리뷰 0회로 통과했습니다. `.git/` 아래는 checkout 이 절대 쓰지 않습니다. 우회 파일 `.mangolove/.review-skip` 만 워킹트리에 남기되, 추적된 사본은 인정하지 않습니다.
>
