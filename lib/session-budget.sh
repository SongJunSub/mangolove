...
#
# 임계 보정(실측, 118세션): 200K 는 오탐 0 을 유지하면서 커버리지가 가장 높다.
#   150K: 발화 65%, 비용커버 99.6%, 그러나 싼 세션($10 미만) 49개 중 6개가 걸려 오탐 12%
#   200K: 발화 54%, 비용커버 98.7%, 오탐 0/49       <- 기본값
#   500K: 발화 29%, 비용커버 91.4%, 오탐 0/49       (과도하게 보수적)
#   Claude Code 자신의 /usage 도 ">150k 컨텍스트"를 고비용 신호로 플래그한다.
#
...
THRESH="${MANGOLOVE_SESSION_BUDGET_TOKENS:-200000}"
case "$THRESH" in
    ''|*[!0-9]*) THRESH=200000 ;;
    *) THRESH=$((10#$THRESH)); [ "$THRESH" -ge 1 ] || THRESH=200000 ;;
esac
...
# 도달한 최고 버킷: THRESH 에서 **배증**한다 (200K, 400K, 800K ...).
# 등차(THRESH/2 씩)로 하면 임계가 낮을수록 알림이 촘촘해진다: 200K 등차는 1M 세션에서
# 9회, 배증은 3회다. 세션이 길어질수록 알림 간격이 벌어지는 쪽이 옳다.
BUCKET="$THRESH"
...
{
    echo "--- MangoLove 세션 예산: 컨텍스트 $((CTX / 1000))K 토큰 (임계 $((THRESH / 1000))K) ---"
    echo "    컨텍스트가 커질수록 매 턴 전체를 재전송하므로 비용이 커지고, 천장에 가까울수록"
    echo "    앞선 지시의 효력이 희석됩니다. 지금이 끊기 좋은 지점인지 사용자에게 알려주세요."
