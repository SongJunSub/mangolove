...
Automatically generated work logs from MangoLove AI Agent sessions.
...
log_session_start() {
    ...
    date '+%Y-%m-%d %H:%M:%S' > "$LOG_DIR/.session_start"
...
log_session_end() {
    ...
    local today now year month file
    today=$(date '+%Y-%m-%d')
    now=$(date '+%H:%M:%S')
    year=$(date '+%Y')
    month=$(date '+%m')
    file="$LOCAL_REPO/logs/${year}/${month}/${today}.md"
