...
- **분석 → 승인**: 분석 결과(수정 파일, 변경 내용, 위험, 엣지케이스)를 사용자에게 제시
- **승인 → 구현**: 사용자가 계획을 명시적으로 승인 ("ㅇㅇ", "진행", "좋아" 등)
- **구현 → 셀프 리뷰**: 빌드/린트/테스트 전체 PASS 확인
- **셀프 리뷰 → 3인 코드 리뷰**: 셀프 리뷰 체크리스트 전항목 PASS
- **3인 코드 리뷰 → 완료 보고**: 3인 리뷰 이슈 전부 해결
...
- 각 에이전트에게 git diff, 변경된 파일 전체, 요구사항(티켓 내용)을 전달하여, 작업 맥락 없이 코드만으로 독립적인 리뷰를 수행한다
