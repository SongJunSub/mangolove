...
## 찾기 → 검증 (find → verify) — 오탐 제거 의무

발견(find)과 검증(verify)을 분리한다. **각 Critical/Warning 발견은 보고하기 전에 스스로
반증(refute)을 시도**한다 — "이 발견이 틀렸다면 왜인가?"를 먼저 답하고, 실제 코드/실행 경로에서
재현 가능한 근거(파일:줄 + 트리거 입력)를 댈 수 있을 때만 확정한다. 근거를 못 대면 폐기하거나
Suggestion으로 강등한다. 그럴듯하나 틀린 지적이 불필요한 수정을 유발하지 않게 한다.
